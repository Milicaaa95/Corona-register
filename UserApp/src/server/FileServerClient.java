...
public class FileServerClient {
	...
	public byte[] getFile(String fileName, String uuid) {
		byte[] data = null;
		try {
	...
	public boolean sendFile(String fileName, byte[] buffer, String uuid, boolean isZip) {
		boolean isOk = false;
		try {
			isOk = fileServer.sendFile(fileName, buffer, uuid, isZip);
		} catch (RemoteException e) {
			logger.log(Level.SEVERE, e.fillInStackTrace().toString());
		}
		return isOk;
